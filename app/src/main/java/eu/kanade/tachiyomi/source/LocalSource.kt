...
class LocalSource(
    ...
    companion object {
        ...
        fun updateCover(
            ...
            val mangaPath = File(dir, manga.url)
            val coverDir =
                if (mangaPath.isFile) {
                    File(dir, "$COVERS_DIR/${manga.url.hashCode()}")
                } else {
                    mangaPath
                }
            var cover = getCoverFile(coverDir)
            if (cover == null) {
                cover = File(coverDir, COVER_NAME)
            }
