...
class ChapterFilter(
    ...
    fun <T : Chapter> filterChapters(
        ...
        return if (readEnabled || unreadEnabled || bookmarkEnabled || notBookmarkEnabled) {
                ...
                filteredChapters
                    ...
                    .map { (_, chapters) ->
                        chapters.find { it.id == selectedChapter?.id }
                            ...
                            ?: chapters.find {
                                val mainScans =
                                    it.scanlator?.split(ChapterUtil.SCANLATOR_SEPARATOR)
                                        ?: return@find false
                                val currScans =
                                    selectedChapter?.scanlator?.split(ChapterUtil.SCANLATOR_SEPARATOR)
                                        ?: return@find false
